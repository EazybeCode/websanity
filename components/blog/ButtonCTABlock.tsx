...
export const ButtonCTABlock: React.FC<{ data: ButtonData }> = ({ data }) => {
  const { text, url, variant = 'primary', size = 'md', openInNewTab = false, alignment = 'left' } = data;
  ...
  const variantStyles: Record<string, string> = {
    primary: 'bg-gradient-to-r from-brand-blue to-brand-cyan text-white hover:shadow-lg hover:shadow-brand-blue/25',
  ...
  return (
    <div className={`my-6 md:my-8 ${alignmentStyles[alignment]}`}>
      <a
        ...
        className={`inline-flex items-center gap-2 rounded-xl font-semibold transition-all duration-200 ${variantStyles[variant]} ${sizeStyles[size]} ${variant !== 'link' ? 'hover:-translate-y-0.5' : ''}`}
      >
        {text}
