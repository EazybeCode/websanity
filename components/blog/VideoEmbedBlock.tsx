...
export const VideoEmbedBlock: React.FC<{ data: VideoData }> = ({ data }) => {
  const { platform, videoId, url, title, autoplay = false, mute = false, controls = true, aspectRatio = '16:9', coverImage } = data;
  ...
  return (
    <div className={`my-8 md:my-12`}>
      {title && <p className="text-sm text-slate-500 mb-3">{title}</p>}
      <div className={`relative w-full rounded-xl overflow-hidden ${aspectRatios[aspectRatio]} bg-slate-900`}>
