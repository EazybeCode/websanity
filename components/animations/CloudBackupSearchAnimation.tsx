...
const CloudBackupSearchAnimation: React.FC = () => {
  ...
  return (
    <div className="relative w-full aspect-[4/3] flex items-center justify-center">
      <div className="w-full h-full bg-brand-black rounded-2xl border border-slate-700 flex flex-col overflow-hidden shadow-2xl">
        {/* Search Header */}
        <div className="p-4 border-b border-slate-700/50 bg-brand-black/50">
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1 bg-brand-card border border-slate-600 rounded-lg flex items-center px-3 py-2">
              <Search className="w-3 h-3 text-blue-400 mr-2" />
              <div className="text-[10px] font-medium text-white flex items-center h-4">
                {searchTerm}<span className="w-[1px] h-3 bg-blue-400 ml-0.5 animate-pulse"></span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t from-brand-black to-transparent pointer-events-none rounded-b-2xl"></div>
      </div>
    </div>
  );
};
