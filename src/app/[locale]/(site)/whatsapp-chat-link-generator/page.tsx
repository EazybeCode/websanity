...
import { ChatLinkGeneratorClient } from '@/components/pages/ChatLinkGeneratorClient'
import { getChatLinkPageContent } from '@/data/whatsapp-chat-link-content'
...
export async function generateMetadata({
  ...
  const t = getChatLinkPageContent(locale)
  return {
    title: t.meta.title,
    description: t.meta.description,
    alternates: getAlternates(locale, '/whatsapp-chat-link-generator'),
...
const ICON_CLICK = (
  <svg width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    ...
    <path d="M7 2v2M2 7h2M4.5 4.5l1.5 1.5M11 4V2" />
  </svg>
)
const ICON_GLOBE = (
  <svg width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export default async function WhatsAppChatLinkGeneratorPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  ...
  const pageUrl = `${SITE_URL}${localePath}/whatsapp-chat-link-generator`
  ...
  const useCases = [
    { title: t.useCases.emailTitle, body: t.useCases.emailBody, icon: ICON_EMAIL },
    { title: t.useCases.bioTitle, body: t.useCases.bioBody, icon: ICON_BIO },
    { title: t.useCases.adsTitle, body: t.useCases.adsBody, icon: ICON_ADS },
  ]
  const benefits = [
  ...
  const benefitCards = [
    { title: t.benefitCards.clickTitle, body: t.benefitCards.clickBody, icon: ICON_CLICK },
    { title: t.benefitCards.everywhereTitle, body: t.benefitCards.everywhereBody, icon: ICON_GLOBE },
  ...
  return (
    <>
        ...
        dangerouslySetInnerHTML={{
          __html: `
            @media (max-width: 640px) {
              .clg-hero-left { text-align: center; }
              .clg-hero-left h1 { margin-left: auto; margin-right: auto; }
              .clg-hero-left p { margin-left: auto; margin-right: auto; }
            }
          `,
        }}
      />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 48, alignItems: 'center' }}>
            <div className="clg-hero-left">
              ...
              <div style={{ marginTop: 28 }}>
                <a href="#chat-link-generator" className="btn btn-primary btn-lg">{t.hero.cta}</a>
              </div>
            ...
            <div style={{ position: 'relative', display: 'flex', justifyContent: 'center' }}>
              ...
              <div style={{ position: 'relative', zIndex: 1, width: 'min(380px, 100%)', background: 'linear-gradient(180deg, #5BAA7F 0%, #4A9170 100%)', borderRadius: 18, padding: 24, color: '#ffffff', boxShadow: '0 20px 50px -16px rgba(15,17,21,0.25), 0 2px 6px rgba(15,17,21,0.08)' }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, background: 'rgba(255,255,255,0.18)', border: '1px solid rgba(255,255,255,0.4)', borderRadius: 999, padding: '10px 0', textAlign: 'center', fontSize: 13, fontWeight: 600 }}>{t.preview.btnCopy}</div>
                </div>
                <div style={{ position: 'absolute', left: -32, bottom: -24, background: '#ffffff', color: '#0F1115', borderRadius: 12, padding: '12px 16px', boxShadow: '0 12px 32px -10px rgba(15,17,21,0.25)', fontFamily: 'var(--f-mono, ui-monospace, monospace)', fontSize: 12, fontWeight: 600, maxWidth: 240 }} aria-label={t.preview.linkAlt}>
                  https://wa.me/13024129610?text=Hi
                </div>
      ...
      {/* Live generator */}
      <section id="chat-link-generator" className="section" style={{ paddingTop: 60, paddingBottom: 80, scrollMarginTop: 80, background: 'linear-gradient(180deg, #ECEFF7 0%, #ffffff 100%)' }}>
        <div className="container" style={{ maxWidth: 1100 }}>
          <div className="sec-head centered" style={{ marginBottom: 32 }}>
            <h2>{t.tryItNow.h2}</h2>
            <p>{t.tryItNow.subtitle}</p>
          </div>
          <ChatLinkGeneratorClient labels={t.generator} />
        </div>
      </section>
      ...
      <section className="section" style={{ paddingTop: 80, paddingBottom: 100, background: '#ffffff', borderTop: '1px solid #E4E8F1' }}>
        <div className="container" style={{ maxWidth: 760, textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="https://eazybe.info/85c80b" target="_blank" rel="noopener noreferrer" className="btn btn-primary">{t.cta.primary}</a>
            <a href="https://calendly.com/d/cw67-pt3-y2m" target="_blank" rel="noopener noreferrer" className="btn" style={{ background: '#5B4BAE', color: '#ffffff' }}>{t.cta.secondary}</a>
          </div>
