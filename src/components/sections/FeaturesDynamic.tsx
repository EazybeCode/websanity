...
import HomeRevenueInboxAnimation from '@/components/animations/HomeRevenueInboxAnimation'
import HomeAnalyticsAnimation from '@/components/animations/HomeAnalyticsAnimation'
import HomeAiAgentAnimation from '@/components/animations/HomeAiAgentAnimation'
...
export const FeaturesDynamic: React.FC<Props> = ({ data }) => {
  ...
  const getFeatureContent = (feature: NonNullable<FeatureSection['features']>[number], index: number) => {
    ...
    if (translationKey) {
      const translatedFeature = t.raw(`home.features.${translationKey}`) as TranslatedFeature | string
