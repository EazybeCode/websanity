...
import type { ProblemSection } from '@/types/sanity'
...
export const ProblemDynamic: React.FC<Props> = ({ data }) => {
  ...
  const badge = isNonEnglish ? t('home.problem.badge', { defaultValue: data.badge || '' }) : (data.badge || t('home.problem.badge', { defaultValue: '' }))
  const headline = isNonEnglish ? t('home.problem.headline', { defaultValue: data.headline || '' }) : (data.headline || t('home.problem.headline', { defaultValue: '' }))
  const subheadline = isNonEnglish ? t('home.problem.subheadline', { defaultValue: data.subheadline || '' }) : (data.subheadline || t('home.problem.subheadline', { defaultValue: '' }))

  // Use translated cards for non-English, Sanity data for English
  const translatedCards = t.raw('home.problem.cards') as Array<{ title: string; description: string }> | string
