  ...
  return {
    ...
    types: {
      image: ({ value }: any) => {
        ...
        // CSS `aspect-ratio` requires "16/9" (slash); Sanity stores "16:9" (colon).
...
const RelatedPostCard: React.FC<{
  ...
  const blogPath = locale === 'en' ? `/comparison/${slug}` : `/${locale}/comparison/${slug}`
...
export const ComparisonPostClient: React.FC<ComparisonPostClientProps> = ({
  ...
  const blogPath = locale === 'en' ? '/comparison' : `/${locale}/comparison`
