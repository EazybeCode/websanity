...
/**
 * Zoho Integration Page SEO - /zoho-whatsapp-integration
 * Adds meta tags and JSON-LD schemas for the Zoho WhatsApp Integration page
 * All schemas are crawlable by bots (no @id for SEO)
 */

export const useZohoIntegrationSEO = () => {
  ...
  useEffect(() => {
    ...
    if (isZohoPage) {
      ...
      // Document title
      document.title = 'Zoho WhatsApp Integration With AI Agents | Sync WhatsApp CRM'
      ...
      // Basic meta tags
      setMetaTag('description', 'Connect WhatsApp with Zoho CRM. Sync chats automatically, use AI agents, track deals, and manage sales conversations directly inside Zoho.')
      setMetaTag('keywords', 'Zoho WhatsApp integration, WhatsApp Zoho CRM, sync WhatsApp with Zoho, Zoho WhatsApp automation, WhatsApp CRM Zoho, AI agents Zoho WhatsApp')
      ...
      setMetaTag('article:tag', 'Zoho WhatsApp Integration', true)

      // Open Graph tags
      setMetaTag('og:type', 'website', true)
      setMetaTag('og:url', 'https://eazybe.com/zoho-whatsapp-integration', true)
      setMetaTag('og:title', 'Zoho WhatsApp Integration With AI Agents | Eazybe', true)
      setMetaTag('og:description', 'Sync WhatsApp with Zoho CRM automatically. Track deals, use AI replies, manage chats, and boost sales productivity directly inside Zoho.', true)
      setMetaTag('og:image', 'https://eazybe.com/logo.png', true)
      setMetaTag('og:image:width', '1200', true)
      setMetaTag('og:image:height', '630', true)
      setMetaTag('og:image:alt', 'Eazybe Zoho WhatsApp Integration Platform', true)
      ...
      setMetaTag('twitter:title', 'Zoho WhatsApp Integration | Sync CRM With WhatsApp', true)
      setMetaTag('twitter:description', 'Automatically sync WhatsApp chats with Zoho CRM. Use AI agents, track pipeline activity, and manage customer conversations in one place.', true)
      setMetaTag('twitter:image', 'https://eazybe.com/logo.png', true)
      setMetaTag('twitter:image:alt', 'Zoho WhatsApp CRM Integration by Eazybe', true)
      ...
      setMetaTag('target-audience', 'Zoho users, sales teams, CRM managers, marketing automation teams, B2B businesses')
      setMetaTag('content-intent', 'commercial-investigation, transactional')
      setMetaTag('conversational-query', 'how to connect WhatsApp to Zoho, best Zoho WhatsApp integration, sync WhatsApp with Zoho CRM')
      setMetaTag('ai-readability', 'conversational, professional, solution-oriented')
      setMetaTag('context-window', 'Zoho automation, WhatsApp CRM sync, deal tracking, sales pipeline management, WhatsApp inside Zoho')
      setMetaTag('user-problem', 'Zoho not connected to WhatsApp, missing WhatsApp leads, manual CRM updates')
      setMetaTag('solution-summary', 'automatic WhatsApp to Zoho synchronization with AI automation')
      setMetaTag('primary-benefit', 'manage WhatsApp conversations directly inside Zoho')
      setMetaTag('use-case', 'sales teams syncing WhatsApp conversations with Zoho CRM automatically')
      setMetaTag('implementation-difficulty', 'easy, one-click Zoho integration')
      ...
      const faqSchema = {
        ...
        "mainEntity": [
          ...
          {
            "@type": "Question",
            "name": "Which Zoho modules can I associate WhatsApp conversations with?",
            "acceptedAnswer": {
              "@type": "Answer",
      ...
      const organizationSchema = {
        ...
        "knowsAbout": ["WhatsApp CRM", "Zoho WhatsApp integration", "Sales automation", "CRM integration", "AI agents for CRM", "Customer engagement"]
      }
      ...
      const webpageSchema = {
        ...
        "@type": "WebPage",
        "url": "https://eazybe.com/zoho-whatsapp-integration",
        "name": "Zoho WhatsApp Integration With AI Agents | Sync WhatsApp CRM",
      ...
      addJsonLdSchema(faqSchema, 'faq-zoho')
      addJsonLdSchema(breadcrumbSchema, 'breadcrumb-zoho')
      addJsonLdSchema(organizationSchema, 'organization-zoho')
      addJsonLdSchema(webpageSchema, 'webpage-zoho')
      addJsonLdSchema(softwareApplicationSchema, 'software-zoho')
      addJsonLdSchema(productSchema, 'product-zoho')
      addJsonLdSchema(howToSchema, 'howto-zoho')

      // Cleanup function - remove meta tags and schema when leaving the page
      return () => {
        // Remove FAQ schema
        const faqScript = document.querySelector('script[type="application/ld+json"][data-schema="faq-zoho"]')
        if (faqScript) faqScript.remove()
        // Remove breadcrumb schema
        const breadcrumbScript = document.querySelector('script[type="application/ld+json"][data-schema="breadcrumb-zoho"]')
        if (breadcrumbScript) breadcrumbScript.remove()
        // Remove organization schema
        const orgScript = document.querySelector('script[type="application/ld+json"][data-schema="organization-zoho"]')
        if (orgScript) orgScript.remove()
        // Remove webpage schema
        const webpageScript = document.querySelector('script[type="application/ld+json"][data-schema="webpage-zoho"]')
        if (webpageScript) webpageScript.remove()
        // Remove software application schema
        const softwareAppScript = document.querySelector('script[type="application/ld+json"][data-schema="software-zoho"]')
        if (softwareAppScript) softwareAppScript.remove()
        // Remove product schema
        const productScript = document.querySelector('script[type="application/ld+json"][data-schema="product-zoho"]')
        if (productScript) productScript.remove()
        // Remove how-to schema
        const howToScript = document.querySelector('script[type="application/ld+json"][data-schema="howto-zoho"]')
